...
local function TweenObject(obj, duration, properties)
    game.TweenService:Create(obj, TweenInfo.new(duration, Enum.EasingStyle.Back, Enum.EasingDirection.InOut), properties):Play()
end
...
AddUICorner(20, MainFrame)

local LogoImage = CreateObject("ImageLabel", {
    Name = "Logo",
    Parent = MainFrame,
    BackgroundTransparency = 1,
    Image = "rbxassetid://93927358445739", 
    Position = UDim2.new(0.5, -50, 0, 20),
    Size = UDim2.new(0, 100, 0, 100)
})
AddUICorner(10, LogoImage)
...
local TitleLabel = CreateObject("TextLabel", {
    ...
    TextColor3 = LoaderConfig.Colors.Text,
    TextSize = 24,
...
local ProgressFrame = CreateObject("Frame", {
    Name = "ProgressFrame",
    Parent = MainFrame,
    BackgroundColor3 = LoaderConfig.Colors.Background,
    BorderSizePixel = 0,
    Position = UDim2.new(0.5, -150, 0, 200),
    Size = UDim2.new(0, 300, 0, 30)
})
AddUICorner(20, ProgressFrame)

local ProgressBar = CreateObject("Frame", {
    Name = "ProgressBar",
    Parent = ProgressFrame,
    BackgroundColor3 = LoaderConfig.Colors.Accent,
    BorderSizePixel = 0,
    Size = UDim2.new(0, 0, 0, 30)
})
AddUICorner(15, ProgressBar)
...
local function UpdateStepText(step, percentage)
    StepLabel.Text = "Loading... " .. percentage .. "% (" .. step .. " of 4)"
end

local function UpdateProgress(percentage, step)
    TweenObject(ProgressBar, 1, { Size = UDim2.new(percentage / 100, 0, 0, 30) })
    UpdateStepText(step, percentage)
end

TweenObject(MainFrame, 1, { Size = UDim2.new(0, 400, 0, 300) })
wait(0.5)

TweenObject(TitleLabel, 1, { TextTransparency = 0 })
TweenObject(ProgressFrame, 1, { BackgroundTransparency = 0 })
TweenObject(ProgressBar, 1, { BackgroundTransparency = 0 })
wait(0.5)

for index, frame in ipairs(LoaderConfig.Keyframes) do
    wait(frame[1]) 
    UpdateProgress(frame[2], index)
end

UpdateProgress(100, #LoaderConfig.Keyframes)

TweenObject(TitleLabel, 1, { TextTransparency = 1 })
TweenObject(ProgressFrame, 1, { BackgroundTransparency = 1 })
TweenObject(ProgressBar, 1, { BackgroundTransparency = 1 })
TweenObject(StepLabel, 1, { TextTransparency = 1 })

wait(1)
TweenObject(MainFrame, 1, { Size = UDim2.new(0, 0, 0, 0) })
wait(0.5)
CoreGui:Destroy()
wait(1)
local sound = Instance.new("Sound") 
sound.SoundId = "rbxassetid://6768022063" 
sound.Volume = 100000000000000000000000000000000 
sound.Looped = false
...
if currentTime - lastNotificationTime >= notificationCooldown then
    game.StarterGui:SetCore("SendNotification", {
        Title = "Greenz Hub",
	    Text = "Loading...",
	    Icon = "rbxthumb://type=Asset&id=93927358445739&w=150&h=150",
	    Duration = 10
    })
...
Server = Window:AddTab({ Title = "Tab Status And Server", Icon = "clock" })
Server:AddButton({
    Title = "Copy discord invite link",
    Callback = function()
        pcall(function()
            setclipboard("https://discord.gg/fRg2P9CpzW")
        end)
    end
})
local Usserrrrr = Server:AddParagraph({
    Title="Player Status",
    Content="━━━━━━━━━━━━━━━━━━━━━\n"..
...
Main = Window:AddTab({ Title = "Tab Farming", Icon = "scroll" })
                   ...
                   for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                       if v.Name == "Isle Outlaw" or v.Name == "Island Boy" or v.Name == "Isle Champion" or v.Name == "Serpent Hunter" or v.Name == "Skull Slayer" then
                           if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                               ...
                               until not _G.FarmDaiBan or not v.Parent or v.Humanoid.Health <= 0 or game:GetService("Workspace").Map.CakeLoaf.BigMirror.Other.Transparency == 0 or game:GetService("ReplicatedStorage"):FindFirstChild("Tyrant of the Skies") or game:GetService("Workspace").Enemies:FindFirstChild("Tyrant of the Skies")
                               DamageAura = false
                           end
                       end
                   end
...
local ToggleMobAuraFarm = Main:AddToggle("ToggleMobAuraFarm", {
    Title = "Auto Farm Near Mob",
    Description = "",
    Default = false
})

ToggleMobAuraFarm:OnChanged(function(Value)
    getgenv().AutoNear = Value
    StopTween(getgenv().AutoNear)
end)
task.spawn(function()
	while task.wait() do
		pcall(function()
			if ((_G.FarmNearest and AutoFarmNearestMagnet) or (SelectMag and getgenv().BringMonster)) then
			...
			for v1645, v1646 in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
				if (v1646.Name and v1646:FindFirstChild("Humanoid")) then
					if (v1646.Humanoid.Health > 0) then
						...
						until  not getgenv().AutoNear or  not v1646.Parent or (v1646.Humanoid.Health <= 0)
...
Main:AddSection("Mastery Farming")
...
Stack = Window:AddTab({ Title = "Tab Stack Farm", Icon = "layers" })
Stack:AddSection("Stack Farming")
...
Toggle = Stack:AddToggle("Toggle", {Title = "Auto Farm Tyrant of the Skies", Default = false })
Toggle:OnChanged(function(Value)
    getgenv().DaiBansss = Value
    StopTween(getgenv().DaiBansss)
end)
      local TyrantoftheSkies = CFrame.new(-16194.0048828125, 155.21844482421875, 1420.719970703125)
local Plsmon = game:GetService("Workspace").Enemies
task.spawn(function()
while task.wait() do
   if getgenv().DaiBansss then
       pcall(function()
           if game:GetService("Workspace").Enemies:FindFirstChild("Tyrant of the Skies") then
               for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                   if v.Name == "Tyrant of the Skies" then
...
Other = Window:AddTab({ Title = "Tab Farming Other", Icon = "boxes" })
Other:AddParagraph({
        Title = "Quest Dragon",
        Content = ""
})
...
local ToggleCollectBerry = Other:AddToggle("ToggleCollectBerry", {
    ...
    while wait() do
        if getgenv().AutoCollectBerry then
            ...
            if Nearest then
                ...
                Fluent:Notify({
                    Title = "Greenz Hub",
                    Content = "Berry Found: " .. tostring(BerryName),
                    Duration = 10
                })
            else
                Hop()
...
ToggleBypassChest = Other:AddToggle("ToggleChest", {Title = "Auto Bypass Chest", Default = false })
ToggleBypassChest:OnChanged(function(Value)
    getgenv().AutoFarmChestBypass = Value
    StopTween(getgenv().AutoFarmChestBypass)
end)
spawn(function()
	while wait() do
		if getgenv().AutoFarmChestBypass then
...
LGa = Window:AddTab({ Title = "Tab Local Players", Icon = "user" })
LGa:AddButton({
    Title = "Stop Tween",
    Callback = function()
        StopTween()
...
DropdownStats = LGa:AddDropdown("Dropdown", {
    Title = "Point Stats",
    Values = {"1", "3", "10", "100", "300"},
    Multi = false,
    Default = 1,
})
...
Toggle = LGa:AddToggle("Toggle", {Title = "Auto Rejoin On Kick", Default = true })
Toggle:OnChanged(function(Value)
...
Sea = Window:AddTab({ Title = "Tab Sea Event", Icon = "calendar-clock" })
...
local function removeDistortion()
    ...
    for _, gui in pairs(playerGui:GetChildren()) do
        if gui:IsA("ScreenGui") and gui.Name:lower():find("distortion") then
            gui.Enabled = false
        end
    end
end
Sea:AddButton({
    Title="Remove Sea Blur Effect",
    Description="",
    Callback=function()
        removeDistortion()
    end
...
Sea:AddSection("Leviathan Event")    
Sea:AddButton({
    Title = "Buy Spy",
    Callback = function()
...
ChooseSkill = Window:AddTab({ Title = "Tab Setting Select Skill", Icon = "list" })
ChooseSkill:AddSection("Setting Sea Event")    
...
ChooseSkill:AddSection("Setting Skill Fruit")    
Toggle = ChooseSkill:AddToggle("Toggle", {
    Title = "Skill Fruit Z",
    Default = true
})
...
ChooseSkill:AddSection("Setting Skill Melee")    
Toggle = ChooseSkill:AddToggle("Toggle", {
    Title = "Skill Melee Z",
    Default = true
})
...
ChooseSkill:AddSection("Setting Skill Sword/Gun")    
Toggle = ChooseSkill:AddToggle("Toggle", {
...
Button.Image = "rbxassetid://93927358445739"
...
local rotateTween = TweenService:Create(Button, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 360})
...
Button.MouseButton1Down:Connect(function()
    ...
    local zoomTween = TweenService:Create(Button, TweenInfo.new(0.2, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {Size = UDim2.new(0, 60, 0, 60)})
    zoomTween:Play()
    zoomTween.Completed:Connect(function()
        local resetZoom = TweenService:Create(Button, TweenInfo.new(0.2, Enum.EasingStyle.Bounce, Enum.EasingDirection.Out), {Size = UDim2.new(0, 50, 0, 50)})
        resetZoom:Play()
    end)
end)
...
Volcanic = Window:AddTab({ Title = "Tab Volcanic Event", Icon = "sailboat" })
Volcanic:AddSection("Main Volcano Island")    
Volcanic:AddButton({
    Title="Tween To Hunt Zone",
    Description="",
    Callback=function()
        topos(CFrame.new(-16917.154296875, 7.757596015930176, 511.8203125))
    end
})
local seatHistory = {}
...
local DropdownBoat = Volcanic:AddDropdown("DropdownBoat", {
    Title="Choose Boat",
    Description="",
    Values=boatList,
    Multi=false,
...
Volcanic:AddButton({
    Title="Tween To Boat",
    Description="Only Where You Buy!",
    Callback=function()
        tpToMyBoat()
    end
})
local TogglePre = Volcanic:AddToggle("MyToggle", {Title = "Auto Find Prehistoric Island", Default = false })
    ...
    while wait() do
        pcall(function()
            if _G.BoatPrehis then
                ...
                if not MyBoat then
                    buyb = topos(CFrame.new(-16933.3535, -0.591003418, 520.89801))
...
Toggle = Volcanic:AddToggle("MyToggle", {Title = "Defend Prehistoric Island", Default = false })
Toggle:OnChanged(function(Value)
...
Volcanic:AddSection("Setting Skill Defend")    
Toggle = Volcanic:AddToggle("Toggle", {
...
Get = Window:AddTab({ Title = "Tab Get Items And Buy", Icon = "wrench" })
...
ToggleTT = Get:AddToggle("Toggle", {Title = "Auto Holy Torch", Default = false})
ToggleTT:OnChanged(function(Value)
    getgenv().HolyTorch = Value
    StopTween(getgenv().HolyTorch)
end)
spawn(function()
	while wait(0.5) do
		pcall(function()
...
PVP = Window:AddTab({ Title = "Tab PVP", Icon = "swords" })
local Playerslist = {}
...
Toggle = PVP:AddToggle("Toggle", {Title = "ESP Island", Default = false })
Toggle:OnChanged(function(Value)
    IslandESP = Value
    if IslandESP then
...
Toggle = PVP:AddToggle("Toggle", {Title = "ESP Fruit", Default = false })
Toggle:OnChanged(function(Value)
    DevilFruitESP = Value
    if DevilFruitESP then
        if not updateConnection then
...
Toggle = PVP:AddToggle("Toggle", {Title = "ESP Player", Default = false })
Toggle:OnChanged(function(Value)
    ESPPlayer = Value
    if ESPPlayer then
        if not updateConnection then
                    ...
                    pcall(function()
                        ...
                        for _,fruit in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
                            if string.find(fruit.Name,"Fruit") then
                                local dfName = string.sub(fruit.Name,1,string.len(fruit.Name)-6).."-"..string.sub(fruit.Name,1,string.len(fruit.Name)-6)
                                game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("CommF_"):InvokeServer("StoreFruit",dfName,getFruit(fruit.Name))
                            end
                        end
                        for _,df in pairs(character:GetChildren()) do
                            if string.find(df.Name,"Fruit") then
                                local dfName = string.sub(df.Name,1,string.len(df.Name)-6).."-"..string.sub(df.Name,1,string.len(df.Name)-6)
                                game:GetService("ReplicatedStorage"):FindFirstChild("Remotes"):FindFirstChild("CommF_"):InvokeServer("StoreFruit",dfName,getFruit(df.Name))
                            end
                        end
                    end)
                end
...
Race = Window:AddTab({ Title = "Tab Upgrade Race", Icon = "anchor" })
...
if getgenv().TrialTeleportDraco then
    topos(CFrame.new(-140208.438, 129.168396, 14352.6025, 0.993527889, -0, -0.113588177, 0, 1, -0, 0.113588177, 0, 0.993527889))
end
Race:AddSection("Normal Races")    
...
Race:AddButton({
    ...
    Callback = function()
		topos(CFrame.new(3034.90527, 2281.01904, -7322.22852, -0.847308636, -5.42493765e-08, 0.53110081, -6.41282227e-08, 1, -1.63851294e-10, -0.53110081, -3.41973845e-08, -0.847308636));
   end
}) 
Race:AddButton({
...
Race:AddButton({
    ...
    Description = "Must Be In Timple Of Time",
    Callback = function()
...
Settings = Window:AddTab({ Title = "Tab Misc", Icon = "settings" })
local plr = game.Players.LocalPlayer

local noFallEnabled = false
local noFallCoroutine = nil

local function runNoFall()
        ...
        if character then
            ...
            if humanoid and rootPart then
                if humanoid:GetState() == Enum.HumanoidStateType.Freefall then
                    rootPart.Velocity = Vector3.new(0, -0.00000000001, 0)
                end
            end
        end
    end
...
Toggle = Settings:AddToggle("Toggle", {Title = "No Clip", Default = true })
...
Settings:AddToggle("NoFallToggle", {
    Title = "Anti Fall",
    Description = "No Fall Down",
    Default = true,
    Callback = function(value)
        noFallEnabled = value
        if noFallEnabled then
            if not noFallCoroutine or coroutine.status(noFallCoroutine) == "dead" then
                noFallCoroutine = coroutine.create(runNoFall)
                coroutine.resume(noFallCoroutine)
            end
        else
            noFallEnabled = false
        end
...
ToggleInstice = Settings:AddToggle("Toggle", {Title = "Auto Turn On Instict", Default = false })
ToggleInstice:OnChanged(function(Value)
    getgenv().AUTOHAKIINSTICT = Value
end)
spawn(function()
    while wait() do
        pcall(function()
            if getgenv().AUTOHAKIINSTICT then
                repeat
                    task.wait();
...
Toggle = Settings:AddToggle("Toggle", {Title = "Bypass Teleport", Default = false })
Toggle:OnChanged(function(Value)
    BypassTP = Value 
end)
...
local function optimizeGraphicsAndSkills()
    ...
    for _, obj in pairs(workspace:GetDescendants()) do
        ...
        if obj:IsA("Sparkles") or obj:IsA("Fire") or obj:IsA("Smoke") or obj:IsA("Beam") then
            obj.Enabled = false
        end
    end
    ...
    workspace.DescendantAdded:Connect(function(obj)
        if obj:IsA("ParticleEmitter") or obj:IsA("Trail") or 
           obj:IsA("Sparkles") or obj:IsA("Fire") or obj:IsA("Smoke") or obj:IsA("Beam") then
            obj.Enabled = false 
        end
    end)
            ...
            for _, part in pairs(workspace:GetChildren()) do 
                if part:IsA("BasePart") and not part:IsDescendantOf(character) then
                    ...
                    elseif distance < 800 then 
                        if part.Anchored and not part:IsA("Terrain") then
                            part.Anchored = false
                        end
                    end
                end
...
Settings:AddButton({
    Title="Fix Lag 70% Graphic",
    Description="",
    Callback=function()
        optimizeGraphicsAndSkills()
    end
})
