...
local function autoHopIfNeeded()
    ...
    while attempt <= maxAttempts do
        ...
        statusText.Text = "Status: No rip_indra True Form server found (Attempt " .. attempt .. "/" .. maxAttempts .. ")"
        warn("No JobId found for hopping")
        attempt = attempt + 1
        task.wait(1)
    end
    statusText.Text = "Status: Failed to find rip_indra True Form server"
end
            ...
            pcall(function()
                ...
                for _, v in pairs(enemies:GetChildren()) do
                    if v.Name == "rip_indra True Form" and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                        foundBoss = true
                        Converted["_TextLabel1"].Text = "Status: Fighting rip_indra True Form"
                        repeat
                            task.wait()
                ...
                if not foundBoss then
                    ...
                    if rip then
                        Converted["_TextLabel1"].Text = "Status: Moving to rip_indra True Form"
