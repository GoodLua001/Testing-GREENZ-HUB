...
TeleportToTarget = function(targetCFrame) if (targetCFrame.Position - plr.Character.HumanoidRootPart.Position).Magnitude > 1000 then topos(targetCFrame)else topos(targetCFrame)end end
...
Tabs.Vol:AddSection("Prehistoric")
local findpre = Tabs.Vol:AddToggle("findpre", {
                ...
                if not workspace["_WorldOrigin"].Locations:FindFirstChild("Prehistoric Island", true) then                
                    ...
                    if not myBoat then
                        ...
                        if plr.Character.Humanoid.Sit == false then
                            ...
                            repeat 
                                wait()
                                targetDestination = CFrame.new(-10000000, 31, 37016.25)
                            ...
                            until 
                                not _G.Prehis_Find or 
                                (targetDestination.Position - plr.Character.HumanoidRootPart.Position).Magnitude <= 10 or 
                                workspace["_WorldOrigin"].Locations:FindFirstChild("Prehistoric Island") or 
                                plr.Character.Humanoid.Sit == false
                            plr.Character.Humanoid.Sit = false
                        end
                    end
                else
                    local islandLocation = workspace["_WorldOrigin"].Locations:FindFirstChild("Prehistoric Island")
                    if (islandLocation.CFrame.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude >= 2000 then 
                        _tp(islandLocation.CFrame)
                    end
                    if workspace.Map:FindFirstChild("PrehistoricIsland", true) or islandLocation then
                        Fluent:Notify({
                            Title = "Notification",
                            Content = "Prehistoric Spawner!!!",
                            Duration = 5,
                        })
                        wait(2)          
                    end
