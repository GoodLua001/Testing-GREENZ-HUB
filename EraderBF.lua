...
Attack.Kill = function(model,Succes)
  ...
  if ToolTip == "Blox Fruit" then _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,10,0) * CFrame.Angles(0,math.rad(90),0)) else _tp(model.HumanoidRootPart.CFrame * CFrame.new(0,30,0) * CFrame.Angles(0,math.rad(180),0))end
	if RandomCFrame then
			task.wait(0.01)_tp(model.HumanoidRootPart.CFrame * CFrame.new(0, 30, 25))
			task.wait(0.01)_tp(model.HumanoidRootPart.CFrame * CFrame.new(25, 30, 0))
			task.wait(0.01)_tp(model.HumanoidRootPart.CFrame * CFrame.new(-25, 30 ,0))
			task.wait(0.01)_tp(model.HumanoidRootPart.CFrame * CFrame.new(0, 30, 25))
			task.wait(0.01)_tp(model.HumanoidRootPart.CFrame * CFrame.new(-25, 30, 0))
		end
...
elseif a>=2726 and a<=2750 then
    ...
    PosQ=CFrame.new(9594.49219, -1943.95752, 9793.58594, 0.965929627, 0, 0.258804798, 0, 1, 0, -0.258804798, 0, 0.965929627)
...
elseif a>=2751 and a<=2800 then
    Mon="Grand Devotee"
    Qdata=2; Qname="SubmergedQuest3"; NameMon="Grand Devotee"
    PosQ=CFrame.new(9594.49219, -1943.95752, 9793.58594, 0.965929627, 0, 0.258804798, 0, 1, 0, -0.258804798, 0, 0.965929627)
...
SME:AddToggle({
    ["Title"] = "Auto Farm Celestial Token",
    ["Title2"] = "",
    ["Content"] = "",
    ["Default"] = false,
    ["Callback"] = function(Value)
        _G.FarmCelestialToken = Value
end
})
spawn(function()
    local CFrameTpOni
...
SME:AddToggle({
    ["Title"] = "Auto Farm Rip Commander",
    ["Title2"] = "Turn On When Join Celestial Domain (Interior)",
    ["Content"] = "",
    ["Default"] = false,
    ["Callback"] = function(Value)
...
local tableboss,bossvalue
if World1 then
    tableboss = {"The Saw", "The Gorilla King", "Bobby", "Yeti", "Mob Leader", "Vice Admiral", "Warden", "Chief Warden", "Swan", "Magma Admiral", "Fishman Lord", "Wysper", "Thunder God", "Cyborg", "Saber Expert"}
    bossvalue = {"The Saw"}
elseif World2 then
    tableboss = {"Diamond", "Jeremy", "Fajita", "Don Swan", "Smoke Admiral", "Cursed Captain", "Darkbeard", "Order", "Awakened Ice Admiral", "Tide Keeper"}
    bossvalue = {"Diamond"}
elseif World3 then
    tableboss = {"Stone", "Island Empress", "Rocket Admiral", "Captain Elephant", "Beautiful Pirate", "rip_indra True Form", "Longma", "Soul Reaper", "Cake Queen", "Cake Prince", "Dough King"}
    bossvalue = {"Stone"}
end
Stack:AddDropdown({
    ["Title"] = "Select Boss",
...
spawn(function()
    while task.wait() do
        if _G.FarmBoss then
            pcall(function()
                local v = GetConnectionEnemies(_G.SelectBoss)
                if v and v:FindFirstChild("Humanoid") then
                    repeat 
                        task.wait()
...
local yinerngu = Cak:AddSection("Yama + Tushita")
yinerngu:AddToggle({
    ["Title"] = "Auto Get Yama",
    ["Title2"] = "",
    ["Content"] = "",
    ...
    pcall(function()
      if _G.Auto_Tushita then
        if workspace.Map.Turtle:FindFirstChild("TushitaGate") then
          ...
          if v then repeat task.wait() Attack.Kill(v, _G.Auto_Tushita) AutoHaki() EquipWeapon(_G.SelectWeapon) until v.Humanoid.Health <= 0 or not _G.Auto_Tushita or not v.Parent
          else 
...
local memayyiner = Cak:AddSection("Berry")
memayyiner:AddToggle({
...
local lon = Cak:AddSection("Factory")
lon:AddToggle({
    ["Title"] = "Auto Factory",
    ["Title2"] = "",
    ["Content"] = "",
...
local MP = FlurioreGui:CreateTab({
	["Name"] = "Tab Volcano",
	["Icon"] = "rbxassetid://7733960981"
})
local Kac = MP:AddSection("Prehistoric")
Kac:AddToggle({
